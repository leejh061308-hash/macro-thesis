...
function hasAdvancedFilters(advanced: AdvancedFilters): boolean {
  return Object.entries(advanced).some(([, v]) => {
...
export default function ScreenerSection({
  ...
  const macroFilters = useMemo(() => {
    const fromThemes = themes as MacroFilter[];
    return [...new Set([...fromThemes, ...extraMacro])];
  }, [themes, extraMacro]);

  const buildRequest = useCallback((): ScreenerRequest => {
    const useAdvanced = hasAdvancedFilters(advanced);
    return {
      mode: useAdvanced ? "advanced" : "beginner",
      beginner: {
        themes,
        macro: macroBeginner,
      },
      strategies: selectedStrategyId ? [selectedStrategyId] : undefined,
      macroFilters: macroFilters.length > 0 ? macroFilters : undefined,
      advanced: useAdvanced ? advanced : undefined,
      sort,
      sortDir,
      limit: 40,
    };
  }, [themes, macroBeginner, selectedStrategyId, macroFilters, advanced, sort, sortDir]);
  ...
  const handleBasicSearch = () => runSearch(buildRequest());
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- 전략 선택 시에만 자동 검색
  }, [selectedStrategyId]);
  ...
  return (
    <div className="space-y-4">
      <p className="text-xs text-gray-400">
        전략을 고르고 테마·거시 조건을 추가한 뒤 검색하세요. 아래에서 지표를 직접 입력해 고급 검색도 할 수 있습니다.
      </p>
      ...
      >
        {loading ? "검색 중..." : "선택 조건으로 검색"}
      </button>

      <div className="rounded-xl border border-surface-border bg-surface-card p-4 space-y-3">
        <div>
          <h3 className="text-sm font-semibold text-white">고급 조건 직접 검색</h3>
          <p className="mt-0.5 text-[11px] text-gray-400">
            PER, ROE, 모멘텀 등 지표를 직접 입력해 검색합니다.
          </p>
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="w-full rounded-lg border border-accent/40 bg-accent/15 py-2.5 text-sm font-semibold text-accent disabled:opacity-50"
        >
          {loading ? "검색 중..." : "고급 조건으로 검색"}
        </button>

        <div className="border-t border-surface-border pt-3 space-y-2">
          <p className="text-[11px] font-semibold text-gray-300">AI 자연어 검색</p>
          <textarea
            value={aiQuery}
            onChange={(e) => setAiQuery(e.target.value)}
            rows={3}
            placeholder='예: "저평가 AI 기업" · "배당 높고 부채 낮은 기업"'
            className="w-full rounded-lg border border-surface-border bg-surface px-3 py-2 text-sm text-white placeholder:text-neutral"
          />
          <button
            type="button"
            onClick={handleAiSearch}
            disabled={loading || !aiQuery.trim()}
            className="w-full rounded-lg border border-surface-border py-2 text-sm text-neutral disabled:opacity-40"
          >
            AI 스크리닝 실행
          </button>
        </div>
