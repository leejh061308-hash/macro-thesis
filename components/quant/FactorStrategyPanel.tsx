...
const UNIVERSE_OPTIONS: { value: UniverseId; label: string }[] = [
  { value: "combined", label: "S&P500 + Nasdaq100" },
...
export default function FactorStrategyPanel({
  ...
  return (
    <div className="space-y-4 rounded-xl border border-surface-border bg-surface-card p-4 card-glow">
      <div>
        <h3 className="text-sm font-bold text-white">멀티팩터 전략</h3>
      ...
      {selectedId !== "custom" && !customMode && (
        <div className="rounded-lg border border-accent/20 bg-accent/5 p-3">
          <p className="text-[11px] leading-relaxed text-gray-300">
            {strategies.find((s) => s.id === selectedId)?.aiSummary}
          </p>
        </div>
      )}
